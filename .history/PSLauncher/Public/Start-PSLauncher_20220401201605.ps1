...
Function Start-PSLauncher {
    ...
    $hash.jsondata = Get-Content $PSLauncherConfigFile | ConvertFrom-Json

    $hash.KeepOpen = $false
    $hash.PanelDraw = 10
    $hash.Color1st = $jsondata.Config.Color1st
    $hash.Color2nd = $jsondata.Config.Color2nd #The darker background for the panels
    $hash.LabelColor = $jsondata.Config.LabelColor
    $hash.TextColor = $jsondata.Config.TextColor
...
'
    ...
    function ShowConsole {
        ...
        $hash.PSConsole = [Console.Window]::GetConsoleWindow()
        [Console.Window]::ShowWindow($hash.PSConsole, 5)
    }
    function HideConsole {
        if (!$hash.KeepOpen) {
            $hash.PSConsole = [Console.Window]::GetConsoleWindow()
            [Console.Window]::ShowWindow($hash.PSConsole, 0)
        }
    }
