...
#>
 ...
 Param (
	...
	<#
Set-LocationToFile
Import-Module ..\PSLauncher.psm1 -Force -Verbose
...
'
	...
	function EnableLogging {
		Start-ShowConsole
		$global:guilogpath = "$env:TEMP\Utilgui-" + (Get-Date -Format yyyy.MM.dd-HH.mm) + '.log'
		Write-Host "creating log $guilogpath"
	...
	foreach ($pan in $panellistSorted) {
		$panel = New-Panel -LabelText $pan.name
		foreach ($but in $data.($pan.name).buttons) {
			...
			New-Button -Text $but.Name -clickAction $clickAction -panel $panel
		}
	}
	#endregion
	...
	$exit.Add_Click( {
			Write-Host 'exiting Util' -ForegroundColor Cyan
			Stop-Process $pid		
		})
	...
	$reload.Add_Click( {
			Write-Host 'Reloading Util' -ForegroundColor Cyan
